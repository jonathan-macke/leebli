package org.leebli.parser.bytecode.utils;
...
/**
 * This class will convert a Ant pattern (containing **) into a regular
 * expression. It allows to run each matching independently.
 * 
 * 
 */
public class AntPatternMatcher {
	...
	static public boolean matches(String expression, String path,
		...
		if (matchers.containsKey(getKey(casesensitive, expression))) {
			return matchers.get(getKey(casesensitive, expression))
					.matches(path);
		} else {
			AntPatternMatcher matcher = new AntPatternMatcher(expression,
					casesensitive);
	...
	public void setExpression(String expression) {
		...
		this.pattern = Pattern.compile(
				AntPatternMatcher.convertToRegexp(expression), flags);
	}
	...
	/**
	 * Convert an Ant Pattern into a regular expression. It supports the
	 * following patterns: '**': matches zero or several subdirs. '*': matches
	 * zero or several characters. '?': matches one character.
	 * 
	 * @param expression
	...
	static public String convertToRegexp(String expression) {
		...
		while (i < n) {
			...
			if (c == '*') {
				...
				result = result + "[^" + File.separator.replace("\\", "\\\\")
						+ "]";
			} else if (c == '[') {
				int j = i;
				...
				if (j >= n) {
					...
					String stuff = expression.substring(i, j).replace("\\",
							"\\\\");
					i = j + 1;
					if (stuff.charAt(0) == '!') {
